...
- **4.3 — Physics engine** (`reasoning/physics.py`): a directional rule table plus closed-form predictors for Currey's law, Frost mechanostat, Paris crack growth, beam bending and stress concentration. Originally used as an IMPLAUSIBLE-only post-filter; in the current Reasoning tab the equations themselves drive hypothesis generation while the rule table serves as the critic's first guard rail.
- **4.4 — Graph cleanup** (`scripts/clean_graph.py` + [`reasoning/graph_cleanup.md`](reasoning/graph_cleanup.md)): six-stage cleanup of the LLM-extracted graph (cross-domain filter, sentence-fragment filter, affix canonicalisation, reverse-pair resolution, low-weight edge drop, orphan removal). 35,338 / 34,265 → **1,597 / 1,699** nodes / edges. Followed by a rule-based concept reclassification pass (`scripts/reclassify_concepts.py` + [`reasoning/graph_concept_reclassification.md`](reasoning/graph_concept_reclassification.md)) that retypes 571 of the 1,200 `concept`-typed nodes into their correct ontology types; the Reasoning pipeline now runs against the cleaned + reclassified graph.
- **4.5 — Physics-driven reasoning** (`reasoning/physics_gen.py`, `reasoning/critic.py`, `reasoning/physics_vars.py`, `reasoning/lrm.py`): the Reasoning tab generates hypotheses *from* physical laws (Currey, Frost mechanostat, Paris, beam bending) and falsifies them with a four-round adversarial physics critic before any output. Predictions carry quantitative ΔY/Y magnitudes; no LLM at runtime; fully deterministic. See [`physics_reasoning.md`](reasoning/physics_grid.md). Supersedes the original Step 4.4 graph-walk design in `phase4_lrm_plan.md`.
- **4.6 — Novelty classifier** (`reasoning/novelty.py`): Two-tier classification. Tier 1: SQLite LIKE keyword search (≥5 hits → GROUNDED, 1–4 → SPECULATIVE, 0 → NOVEL). Tier 2: SPECTER2 cosine similarity against 8,000 randomly sampled corpus embeddings (≥0.82 → GROUNDED, 0.60–0.82 → SPECULATIVE, <0.60 → NOVEL). Final label takes the more conservative (less novel) tier. SPECTER2 model is shared with the retriever to avoid loading 1.6 GB twice. Used as the corpus-grounding pass on every survivor of the physics critic.
- **4.7 — Reasoning tab** (`api/main.py` + `frontend/index.html`): Reasoning tab in the React UI driven by `LRM.query_physics()`. Each chain card shows the law header, a scenario tag, the chain, a two-pane perturbation/prediction box, a green/red critic-rounds pill with falsification reason, and a corpus-grounding novelty badge.

Remaining: LRM benchmark evaluation, and migrating `eval/run_lrm_eval.py` from the legacy `LRM.query()` to `LRM.query_physics()`.
